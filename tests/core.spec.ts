...
describe('computePages', () => {
  ...
  const _test = (limit: number, total: number, reservedPageCount: number) => (
    patterns: (number | PagePosition[])[][]
  ) => {
    describe(`limit: ${limit}, total: ${total}, reservedPageCount: ${reservedPageCount}`, () => {
      describe.each(patterns)('offset: %i', (offset: number, expected: PagePosition[]) => {
        it(`=> ${JSON.stringify(expected)}`, () => {
          expect(computePages(limit, offset, total, reservedPageCount)).toEqual(expected);
        });
      });
    });
  };
